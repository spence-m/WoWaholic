...
export const getClassicHolidaysAfterMessup = (
    year,
    warsongEpoch,
    arathiEpoch,
    alteracEpoch,
    ...
    while (start.getFullYear() <= year) {
        ...
        const warsongDate = new Date(warsongEpoch);
        warsongDate.setDate(warsongDate.getDate() + 21 * i);
        const arathiDate = new Date(arathiEpoch);
        arathiDate.setDate(arathiDate.getDate() + 21 * i);
        const alteracDate = new Date(alteracEpoch);
        alteracDate.setDate(alteracDate.getDate() + 21 * i);
